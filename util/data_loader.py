...
class Sample(FewshotSampleBase):
    def __init__(self, filelines):
        ...
        # lấy word (cột 0) và label (cột cuối)
        self.words = [line[0].lower() for line in filelines]
        self.tags = [line[-1] for line in filelines]
...
def get_loader(filepath, tokenizer, N, K, Q, batch_size, max_length, 
        num_workers=0, collate_fn=collate_fn, ignore_index=-1, use_sampled_data=True):
