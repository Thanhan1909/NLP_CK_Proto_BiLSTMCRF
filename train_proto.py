...
def main():
    ...
    parser.add_argument('--pretrain_ckpt', default=None,
        ...
        # optional explicit data file paths (CoNLL style)
    parser.add_argument('--train_file', default=None,
            help='explicit path to training file (CoNLL format)')
    parser.add_argument('--dev_file', default=None,
            help='explicit path to dev file (CoNLL format)')
    parser.add_argument('--test_file', default=None,
            help='explicit path to test file (CoNLL format)')
    ...
    if not opt.use_sampled_data:
        # choose explicit paths if provided, else prefer data/<mode>/, else data/
        def choose_path(arg_path, mode_fname, mode):
            # nếu user truyền đường dẫn thì dùng luôn
            if arg_path:
                return arg_path
            cand = os.path.join('data', mode_fname)
            if os.path.exists(cand):
                return cand

            # fallback (trường hợp hiếm, vẫn giữ cho chắc)
            cand2 = os.path.join('data', mode, mode_fname)
            if os.path.exists(cand2):
                return cand2

            # nếu không có thì trả về mặc định KHÔNG có inter/intra
            return os.path.join('data', mode_fname)

        opt.train = "D:/Du_Lieu_Hoc/NLP/Few-NERD-main/data/train.txt"
        opt.dev   = "D:/Du_Lieu_Hoc/NLP/Few-NERD-main/data/dev.txt"
        opt.test  = "D:/Du_Lieu_Hoc/NLP/Few-NERD-main/data/test.txt"
        ...
        if not (os.path.exists(opt.train) and os.path.exists(opt.dev) and os.path.exists(opt.test)):
            print('Some data files not found; attempting to download via data/download.sh')
...
# ===== SAVE MODEL FOR DEMO =====
    ...
    print(f" Saved demo model: {save_path}")
